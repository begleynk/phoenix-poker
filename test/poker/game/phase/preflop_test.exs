defmodule Poker.Game.Phase.PreflopTest do
  ...
  test "it transitions only once all bets are matched", %{players: players} do
    ...
    assert {:ok, _} = Game.handle_action(pid, Action.call(amount: 10, position: 1))
    assert {:ok, _} = Game.handle_action(pid, Action.call(amount: 10, position: 2))
    assert {:ok, _} = Game.handle_action(pid, Action.bet(amount: 30, position: 3))
    assert {:ok, _} = Game.handle_action(pid, Action.fold(position: 0))
    assert {:ok, _} = Game.handle_action(pid, Action.call(amount: 20, position: 1))
    assert {:ok, _} = Game.handle_action(pid, Action.call(amount: 20, position: 2))
